// 开始时间：2023-11-22 20:02:09
// 题目标题：网格中的最小路径代价
// 题目标记：minimum-path-cost-in-a-grid
// 题目编号：2304
// 题目描述：
    ...
    public int minPathCost(int[][] grid, int[][] moveCost) {
        ...
        for (int i = 1; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                for (int k = 0; k < n; k++) {
                    int preGridVal = grid[i - 1][k];
                    int dist = moveCost[preGridVal][j];
                    dp[i][j] = Math.min(dp[i][j], dist + dp[i - 1][k] + gridVal);
                }
            }
        }
