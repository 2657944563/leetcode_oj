// 开始时间：2023-09-30 09:27:45
// 题目标题：全部开花的最早一天
// 题目标记：earliest-possible-day-of-full-bloom
// 题目编号：2136
// 题目描述：
...
class Solution {
    public int earliestFullBloom(int[] plantTime, int[] growTime) {
        ...
        for (int f = 0; f < plantTime.length; f++) {
            result[f][0] = plantTime[f];
            result[f][1] = growTime[f];
        }
        Arrays.sort(result, Comparator.comparingInt(o -> -o[1]));
        int start = 0, end = 0;
        for (int[] ints : result) {
            start += ints[0];
            end = Math.max(end, start + ints[1]);
        }
        return end;
    }
}
